...
public class ArrayList<T> implements List<T> {
    ...
    private class ElementsIterator implements ListIterator<T> {
        ...
        public void add(final T element) {
            final T[] newM = (T[])new Object[m.length+1];
            System.arraycopy(m, 0, newM, 0, index);
            System.arraycopy(m, index, newM, index + 1, m.length - index);
            newM[index] = element;
            size++;
            index++;
            m=newM;
        ...
        public void set(final T element) {
            if(lastIndex==LAST_IS_NOT_SET) throw new
...
//
        ...
        @Override
        public T previous() {
            if ((index-1)==-1) throw new NoSuchElementException();
            lastIndex=index;
            return m[--index];
        }
